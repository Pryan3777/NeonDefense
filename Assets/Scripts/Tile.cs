...
public class Tile : MonoBehaviour
{
    ...
    private void OnMouseOver()
    {
        ...
        if (Input.GetMouseButtonDown(1) && !GameManager.Instance.IsPaused() && IsPlacementValid())
        {
            ...
            RemoveStructure();
        } else if (Input.GetMouseButtonDown(0) && !GameManager.Instance.IsPaused() && GameManager.Instance.GetBuildMode() != GameManager.BUILDMODE_NONE && IsPlacementValid() && !GameManager.Instance.IsFireModeEnabled() && !GameManager.Instance.GetRoundStatsEnabled())
        {
            if (ContainsTurret())
            {
            ...
            else if (Input.GetKey(KeyCode.LeftAlt))
            {
                ...
                if (!ContainsWall())
                {
                    ...
                    else
                    {
                        ...
                        GridManager.Instance.MultiFill(-1);
                    }
                }
                else
                {
                    ...
                    {
                        GridManager.Instance.SetSecondTile(this);
                        GridManager.Instance.MultiFill(GameManager.Instance.GetTurretType());
                    }
                }
            }
            else
    ...
    public void RemoveTurret(bool fromPlayer)
    {
        // sell the item
        EconomyManager.Instance.SellItem(_turretInTile);
        // set as player removed turret
        _turretInTile.SetPlayerRemoved(fromPlayer);
	...
	public bool DamageWall(float f)
	{
		...
		{
            if (_wallInTile != null && _wallInTile.Damage(f))
			{
				if(_turretInTile != null && _turretInTile.gameObject != null)
				{
                    RemoveTurret(false);
				}
				return true;
    ...
    public void DisplayHeatMap()
    {
        ...
        get num visited
        get num of enemies that spawned
        # visited / # spawned

        base colors off this percent (as a decimal)
